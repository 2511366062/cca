...
#没读出来
a = geo_download("GSE183904")$pd#单细胞也能用这个下载，主要是要临床信息
head(a)
```
...
if(!file.exists("f.Rdata")){
  ...
  f = lapply(paste0("GSE183904_RAW/",fs),read.csv,row.names = 1)#row.names = 1是csv的参数
...
for(i in 1:length(f)){
  scelist[[i]] <- CreateSeuratObject(counts = f[[i]], 
                                     project = names(f)[[i]])#决定orign.ident
  print(dim(scelist[[i]]))
}
sce.all = merge(scelist[[1]],scelist[-1])
sce.all = JoinLayers(sce.all) #把layer合并，count1 count2
